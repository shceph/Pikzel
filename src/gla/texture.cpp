...
TextureCubeMap::TextureCubeMap(std::array<std::string, 6> paths) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (buffers.at(i) == nullptr) {
            ...
            stbi_image_free(buffers.at(i));
        }
    }
}

TextureCubeMap::~TextureCubeMap() { glDeleteTextures(1, &mTextureID); }
...
void TextureCubeMap::Unbind() const { glBindTexture(GL_TEXTURE_CUBE_MAP, 0); }
...
Texture2D::~Texture2D() { glDeleteTextures(1, &mTextureID); }
