...
void Camera::SetCenter(glm::vec2 center) { mCenter = center; }

void Camera::MoveCenter(glm::vec2 offset) { mCenter += offset; }
...
void Camera::ResetCenter() { mCenter = mCanvasDims / 2; }
...
void Camera::CursorPosCallback(double x_pos, double y_pos) {
    ...
    if (Events::IsMouseButtonHeld(Events::MouseButtons::kButtonRight)) {
        ...
        const glm::vec2 resolution_at_no_additinal_offset{500, 500};
